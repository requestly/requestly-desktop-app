...
const POSIX_OVERRIDE_BIN_PATH =
  process.platform === "win32"
    ? OVERRIDE_BIN_PATH.replace(/\\/g, "/").replace(
        /^(\w+):/,
        (_all, driveLetter) => `/${driveLetter.toLowerCase()}`
      )
    : OVERRIDE_BIN_PATH;

const SHELL = (process.env.SHELL || "").split("/").slice(-1)[0];

const appendOrCreateFile = promisify(fs.appendFile);
...
const START_CONFIG_SECTION = "# --requestly--";
const END_CONFIG_SECTION = "# --requestly-end--";
...
# This section will be reset each time a Requestly terminal is opened
if [ -n "$REQUESTLY_ACTIVE" ]; then
    # When Requestly is active, we inject various overrides into PATH
...
export const getShellScript = (env) => `${_.map(
  env,
  (value, key) => `    export ${key}="${value}"`
).join("\n")}
...
// shells launched with REQUESTLY_ACTIVE set use the interception PATH.
export const editShellStartupScripts = async () => {
  ...
  appendToFirstExisting(
    [path.join(os.homedir(), ".zshenv"), path.join(os.homedir(), ".zshrc")],
  ...
  appendToFirstExisting(
    ...
    SHELL === "fish" ||
      (await canAccess(path.join(os.homedir(), ".config", "fish"))),
...
const removeConfigSectionsFromFile = async (path) => {
  ...
  let sectionStart = _.findIndex(fileLines, (l) =>
    l.startsWith(START_CONFIG_SECTION)
  );
  while (sectionStart !== -1) {
    let sectionEnd = _.findIndex(fileLines, (l) =>
      l.startsWith(END_CONFIG_SECTION)
    );
    ...
    sectionStart = _.findIndex(fileLines, (l) =>
      l.startsWith(START_CONFIG_SECTION)
    );
  }
...
// unless REQUESTLY_ACTIVE is set anyway).
export const resetShellStartupScripts = () => {
  ...
  return Promise.all(
    [
      ...
      removeConfigSectionsFromFile(configFile).catch((e) => {
        Sentry.captureException(e);
        return e.message;
      })
    )
