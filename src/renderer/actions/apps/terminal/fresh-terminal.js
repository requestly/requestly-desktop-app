...
/** ACTIONS */
import { canAccess } from "../../fileManagement";
import { commandExists } from "../../shell/index";
import { spawnToResult } from "../../processManagement";
...
const getTerminalCommand = _.memoize(async () => {
  ...
  result.then((terminal) => {
    if (terminal) console.log("Found terminal", { data: { terminal } });
    else console.error("No terminal could be detected");
  });
  return result;
});
  ...
  const terminalExecutables = (
    await Promise.all(
      [
        ...
        findExecutableById(bundleId).catch((error) => {
...
const getLinuxTerminalCommand = async () => {
  ...
  if (GSettings.isAvailable()) {
    ...
    const defaultTerminal =
      gSettingsTerminalKey && gSettingsTerminalKey.getValue();
...
const getXTerminalCommand = async (command = "x-terminal-emulator") => {
  ...
  try {
    ...
    if (
      helpOutput.includes("gnome terminal") &&
      (await commandExists("gnome-terminal"))
    ) {
      return getGnomeTerminalCommand();
    } else if (
      helpOutput.includes("xfce4 terminal") &&
      (await commandExists("xfce4-terminal"))
    ) {
      return getXfceTerminalCommand();
    } else if (
      helpOutput.includes("konsole") &&
...
const getGnomeTerminalCommand = async (command = "gnome-terminal") => {
  ...
  if (stdout.includes("--wait")) {
    ...
    // extraArgs = ["--app-id", `com.requestly.${randomId}`];
    extraArgs = ["--app-id", `io.requestly.beta`];
  }
...
export class FreshTerminal {
  ...
  async activate(proxyPort) {
    ...
    if (!terminalSpawnArgs)
      throw new Error(
        "Could not find a suitable terminal",
        {cause: appLaunchErrorTypes.APP_ACTIVATION_FAILED}
      );
