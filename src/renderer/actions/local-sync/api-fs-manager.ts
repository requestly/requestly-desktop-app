...
export class APIFsManager {
  private rootPath: string;

  static homeFolder = 'apis';

  constructor(
    parentRootPath: string,
    // eslint-disable-next-line no-unused-vars
    private readonly fsIgnoreManager: FsIgnoreManager,
  ) {
    this.rootPath = `${getNormalizedPath(parentRootPath)}${APIFsManager.homeFolder}`;
  }
  ...
  async createHomeFolder() {
    ...
    const createResult = await createFolder(folderResource, {
